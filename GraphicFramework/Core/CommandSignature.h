...
public:
    ...
    void CreateVertexBufferView(UINT Slot)
    {
...
public:
    ...
    {
        if (NumParams > 0)
            m_ParamArray.reset(new CIndirectParameter[NumParams]);
        else
            m_ParamArray = nullptr;
    ...
    CIndirectParameter& operator[] ( size_t EntryIndex )
    {
        ASSERT(EntryIndex < m_NumParameters);
        return m_ParamArray.get()[EntryIndex];
    }

    const CIndirectParameter& operator[] ( size_t EntryIndex ) const
    {
        ASSERT(EntryIndex < m_NumParameters);
        return m_ParamArray.get()[EntryIndex];
    }
...
protected:
    ...
    std::unique_ptr<CIndirectParameter[]> m_ParamArray;
