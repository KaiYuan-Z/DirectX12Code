...
uint32_t CColor::R11G11B10F(bool RoundToEven) const
{
#if 1
    ...
    R.f = MathUtility::Clamp(m_value.f[0], 0.0f, kMaxVal) * kF32toF16;
    G.f = MathUtility::Clamp(m_value.f[1], 0.0f, kMaxVal) * kF32toF16;
    B.f = MathUtility::Clamp(m_value.f[2], 0.0f, kMaxVal) * kF32toF16;
...
uint32_t CColor::R9G9B9E5() const
{
#if 1
    ...
    float r = MathUtility::Clamp(m_value.f[0], 0.0f, kMaxVal);
    float g = MathUtility::Clamp(m_value.f[1], 0.0f, kMaxVal);
    float b = MathUtility::Clamp(m_value.f[2], 0.0f, kMaxVal);

    // Compute the maximum channel, no less than 1.0*2^-15
    float MaxChannel = MathUtility::Max(MathUtility::Max(r, g), MathUtility::Max(b, kMinVal));
