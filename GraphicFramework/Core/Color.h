...
{
public:
    CColor( ) : m_value(g_XMOne) {}
    CColor( FXMVECTOR vec );
    CColor( const XMVECTORF32& vec );
    CColor( float r, float g, float b, float a = 1.0f );
    CColor( uint16_t r, uint16_t g, uint16_t b, uint16_t a = 255, uint16_t bitDepth = 8 );
    explicit CColor( uint32_t rgbaLittleEndian );
    ...
    bool operator==( const CColor& rhs ) const { return XMVector4Equal(m_value, rhs.m_value); }
    bool operator!=( const CColor& rhs ) const { return !XMVector4Equal(m_value, rhs.m_value); }
    ...
    CColor ToSRGB() const;
    CColor FromSRGB() const;
    CColor ToREC709() const;
    CColor FromREC709() const;
...
INLINE CColor Max( CColor a, CColor b ) { return CColor(XMVectorMax(a, b)); }
INLINE CColor Min( CColor a, CColor b ) { return CColor(XMVectorMin(a, b)); }
INLINE CColor Clamp( CColor x, CColor a, CColor b ) { return CColor(XMVectorClamp(x, a, b)); }


inline CColor::CColor( FXMVECTOR vec )
{
    m_value.v = vec;
}

inline CColor::CColor( const XMVECTORF32& vec )
{
    m_value = vec;
}
...
inline CColor::CColor( uint16_t r, uint16_t g, uint16_t b, uint16_t a, uint16_t bitDepth )
{
    m_value.v = XMVectorScale(XMVectorSet(r, g, b, a), 1.0f / ((1 << bitDepth) - 1));
}

inline CColor::CColor( uint32_t u32 )
{
...
inline CColor CColor::ToSRGB( void ) const
{
...
inline CColor CColor::FromSRGB( void ) const
{
    XMVECTOR T = XMVectorSaturate(m_value);
...
inline CColor CColor::ToREC709( void ) const
{
    XMVECTOR T = XMVectorSaturate(m_value);
...
inline CColor CColor::FromREC709( void ) const
{
    XMVECTOR T = XMVectorSaturate(m_value);
...
inline uint32_t CColor::R10G10B10A2( void ) const
{
...
inline uint32_t CColor::R8G8B8A8( void ) const
{
