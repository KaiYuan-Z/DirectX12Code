...
{
    ASSERT(MathUtility::IsAligned(_Dest, 16));
    ASSERT(MathUtility::IsAligned(_Source, 16));
    ...
    size_t InitialQuadwordCount = (4 - ((size_t)Source >> 4) & 3) & 3;// The last "&3" is to deal with "4 - 0"
...
{
	if (MathUtility::IsAligned(Dest, 16) && MathUtility::IsAligned(Source, 16) && MathUtility::IsAligned(SizeInBytes, 16))
	{
		SIMDMemCopy(Dest, Source, MathUtility::DivideByMultiple(SizeInBytes, 16));
	}
	else
	{
		memcpy(Dest, Source, SizeInBytes);
	}
}
...
std::string MakeStr(const std::wstring& wstr)
{
	...
	size_t _Count = 0;
	wcstombs_s(&_Count, _Dest, _Dsize, _Source, _Dsize);
