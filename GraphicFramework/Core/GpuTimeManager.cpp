...
void GpuTimeManager::Initialize(uint32_t MaxNumTimers)
{
	...
	GraphicsCore::GetCommandQueue().GetCommandQueue()->GetTimestampFrequency(&GpuFrequency);
    ...
    ASSERT_SUCCEEDED(GraphicsCore::GetD3DDevice()->CreateCommittedResource( &HeapProps, D3D12_HEAP_FLAG_NONE, &BufferDesc,
    ...
    ASSERT_SUCCEEDED(GraphicsCore::GetD3DDevice()->CreateQueryHeap(&QueryHeapDesc, MY_IID_PPV_ARGS(&sm_QueryHeap)));
...
void GpuTimeManager::StartTimer(CCommandList* pCommandList, uint32_t TimerIdx)
{
	_ASSERTE(pCommandList);
	pCommandList->InsertTimeStamp(sm_QueryHeap, TimerIdx * 2);
}

void GpuTimeManager::StopTimer(CCommandList* pCommandList, uint32_t TimerIdx)
{
	_ASSERTE(pCommandList);
	pCommandList->InsertTimeStamp(sm_QueryHeap, TimerIdx * 2 + 1);
}

void GpuTimeManager::BeginReadBack(void)
{
	GraphicsCore::WaitForFence(sm_Fence);
...
void GpuTimeManager::EndReadBack(void)
{
	...
	_ASSERTE(pCommandList);
	pCommandList->InsertTimeStamp(sm_QueryHeap, 1);
	pCommandList->ResolveTimeStamps(sm_ReadBackBuffer, sm_QueryHeap, sm_NumTimers * 2);
	pCommandList->InsertTimeStamp(sm_QueryHeap, 0);
	sm_Fence = GraphicsCore::FinishCommandList(pCommandList, false);
}
