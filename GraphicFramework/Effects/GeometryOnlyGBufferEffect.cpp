#include "GeometryOnlyGBufferEffect.h"
#include "../Core/GraphicsCommon.h"

#include "CompiledShaders/GeometryOnlyGBufferEffectVS.h"
#include "CompiledShaders/GeometryOnlyGBufferEffectPS.h"

CGeometryOnlyGBufferEffect::CGeometryOnlyGBufferEffect()
{
}
...
void CGeometryOnlyGBufferEffect::Init(const SGeometryOnlyGBufferConfig& config)
{
	_ASSERTE(config.Width > 0 && config.Height > 0);
	m_SimpleGBufferConfig = config;
	m_ViewPort.TopLeftX = m_ViewPort.TopLeftY = 0;
	m_ViewPort.Width = (float)m_SimpleGBufferConfig.Width;
	m_ViewPort.Height = (float)m_SimpleGBufferConfig.Height;
...
void CGeometryOnlyGBufferEffect::Render(CGraphicsCommandList* pCommandList, bool useAlphaClip)
{
	...
	pCommandList->SetRenderTargets(6, RTVs, m_pCurDepthBuffer->GetDSV());

	m_GBufferRender.DrawScene(pCommandList, useAlphaClip);
}

void CGeometryOnlyGBufferEffect::__InitBuffers()
{
	m_pPrePosBuffer = new CColorBuffer(); _ASSERTE(m_pPrePosBuffer);
	m_pPrePosBuffer->Create2D(L"GBuffer-PosBuffer0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R32G32B32A32_FLOAT);
	m_pPreNormBuffer = new CColorBuffer(); _ASSERTE(m_pPreNormBuffer);
	m_pPreNormBuffer->Create2D(L"GBuffer-NormBuffer0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R16G16_FLOAT);
	m_pPreMotionBuffer = new CColorBuffer(); _ASSERTE(m_pPreMotionBuffer);
	m_pPreMotionBuffer->Create2D(L"GBuffer-MotionBuffer0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R16G16_FLOAT);
	m_pPreDdxyBuffer = new CColorBuffer(); _ASSERTE(m_pPreDdxyBuffer);
	m_pPreDdxyBuffer->Create2D(L"GBuffer-DdxyBuffer0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R16G16_FLOAT);
	m_pPreLinearDepthBuffer = new CColorBuffer(); _ASSERTE(m_pPreLinearDepthBuffer);
	m_pPreLinearDepthBuffer->Create2D(L"GBuffer-LinearDepthBuffer0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R32G32_FLOAT);
	m_pPreIdBuffer = new CColorBuffer(CColor(-1.0f, -1.0f, -1.0f, -1.0f)); _ASSERTE(m_pPreIdBuffer);
	m_pPreIdBuffer->Create2D(L"GBuffer-IdBuffer0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R32G32B32A32_SINT);

	m_pCurPosBuffer = new CColorBuffer(); _ASSERTE(m_pCurPosBuffer);
	m_pCurPosBuffer->Create2D(L"GBuffer-PosBuffer1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R32G32B32A32_FLOAT);
	m_pCurNormBuffer = new CColorBuffer(); _ASSERTE(m_pCurNormBuffer);
	m_pCurNormBuffer->Create2D(L"GBuffer-NormBuffer1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R16G16_FLOAT);
	m_pCurMotionBuffer = new CColorBuffer(); _ASSERTE(m_pCurMotionBuffer);
	m_pCurMotionBuffer->Create2D(L"GBuffer-MotionBuffer1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R16G16_FLOAT);
	m_pCurDdxyBuffer = new CColorBuffer(); _ASSERTE(m_pCurDdxyBuffer);
	m_pCurDdxyBuffer->Create2D(L"GBuffer-DdxyBuffer1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R16G16_FLOAT);
	m_pCurLinearDepthBuffer = new CColorBuffer(); _ASSERTE(m_pCurLinearDepthBuffer);
	m_pCurLinearDepthBuffer->Create2D(L"GBuffer-LinearDepthBuffer1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R32G32_FLOAT);
	m_pCurIdBuffer = new CColorBuffer(CColor(-1.0f, -1.0f, -1.0f, -1.0f)); _ASSERTE(m_pCurIdBuffer);
	m_pCurIdBuffer->Create2D(L"GBuffer-IdBuffer1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, 1, DXGI_FORMAT_R32G32B32A32_SINT);

	m_pPreDepthBuffer = new CDepthBuffer(); _ASSERTE(m_pPreDepthBuffer);
	m_pPreDepthBuffer->Create2D(L"GBuffer-Depth0", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, m_SimpleGBufferConfig.DepthStencilBufferFormat);
	...
	m_pCurDepthBuffer->Create2D(L"GBuffer-Depth1", m_SimpleGBufferConfig.Width, m_SimpleGBufferConfig.Height, m_SimpleGBufferConfig.DepthStencilBufferFormat);
	m_pCurDepthBuffer->SetClearDepth(1.0f);
	m_pCurDepthBuffer->SetClearStencil(0);
}

void CGeometryOnlyGBufferEffect::__InitRootRootSignature()
{
	m_RootSignature.Reset(m_GBufferRender.GetRootParameterCount(), m_GBufferRender.GetStaticSamplerCount());
	m_GBufferRender.ConfigurateRootSignature(m_RootSignature);
	m_RootSignature.Finalize(L"SimpleGBufferEffect", D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
}

void CGeometryOnlyGBufferEffect::__InitPSO()
{
	...
	m_PSO.SetVertexShader(g_pGeometryOnlyGBufferEffectVS,sizeof(g_pGeometryOnlyGBufferEffectVS));
	m_PSO.SetPixelShader(g_pGeometryOnlyGBufferEffectPS, sizeof(g_pGeometryOnlyGBufferEffectPS));
	auto RasterizerState = CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);
	RasterizerState.CullMode = m_SimpleGBufferConfig .OpenBackFaceCulling?  D3D12_CULL_MODE::D3D12_CULL_MODE_BACK : D3D12_CULL_MODE::D3D12_CULL_MODE_NONE;
	...
	m_PSO.SetRenderTargetFormats(6, RenderTargetFormats, m_SimpleGBufferConfig.DepthStencilBufferFormat);
	m_PSO.Finalize();
}

void CGeometryOnlyGBufferEffect::__ReleaseBuffers()
{	
