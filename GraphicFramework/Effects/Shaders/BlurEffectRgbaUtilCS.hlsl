...
Texture2D<float4> g_InputMap        : register(t1);
...
void processSampleInnerHalf(int radius, int r, float2 uv, float centerD, inout float totalGaussW, inout float4 totalColor)
{
...
void processSampleOuterHalf(int radius, int r, float2 uv, float centerD, inout float totalGaussW, inout float4 totalColor)
{
...
void main(uint2 Gid : SV_GroupID, uint2 GTid : SV_GroupThreadID, uint GI : SV_GroupIndex, uint2 DTid : SV_DispatchThreadID)
{
    ...
    float4 color = g_InputMap.SampleLevel(g_samPointClamp, texC, 0.0f);
	...
	float totalColorWeight = centerWeight;
