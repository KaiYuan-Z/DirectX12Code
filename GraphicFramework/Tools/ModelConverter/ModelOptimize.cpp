...
void AssimpModel::__OptimizeRemoveDuplicateVertices(bool depth)
{
...
void AssimpModel::__ptimizePostTransform(bool depth)
{
...
void AssimpModel::__ptimizePreTransform(bool depth)
{
...
void AssimpModel::__Optimize()
{
    // TODO: quantize/compress vertex data

    __OptimizeRemoveDuplicateVertices(false);
    __OptimizeRemoveDuplicateVertices(true);

    // re-order indices for post transform cache
    __ptimizePostTransform(false);
    __ptimizePostTransform(true);

    // re-order vertices for linear memory access
    __ptimizePreTransform(false);
    __ptimizePreTransform(true);
}
