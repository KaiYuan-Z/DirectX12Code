...
void CRayTracingAO::OnRender()
{
	...
	pComputeCommandList->SetBufferSRV(m_UserRootParmStartIndex + 3, m_SampleSets);
	pComputeCommandList->SetDynamicDescriptor(m_UserRootParmStartIndex + 4, 0, m_OutputBuffer.GetUAV());
...
void CRayTracingAO::__InitializeRayTracingResources()
{
	...
	m_RtGlobalRootSignature.Reset(m_SceneRenderHelper.GetRootParameterCount() + 5, m_SceneRenderHelper.GetStaticSamplerCount());
	...
	m_RtGlobalRootSignature[m_UserRootParmStartIndex + 1].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 1, 0, D3D12_SHADER_VISIBILITY_ALL);// (t0)g_Pos
	m_RtGlobalRootSignature[m_UserRootParmStartIndex + 2].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1, 0, D3D12_SHADER_VISIBILITY_ALL);// (t1)g_Norm
	m_RtGlobalRootSignature[m_UserRootParmStartIndex + 3].InitAsBufferSRV(2, 0, D3D12_SHADER_VISIBILITY_ALL);// (t2)g_SampleSets
	m_RtGlobalRootSignature[m_UserRootParmStartIndex + 4].InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 0, 1, 0);// (u0)g_Output
...
void CRayTracingAO::__InitializeRandomSamples()
{
	UINT pixelsInSampleSet1D = 1;
	UINT samplesPerSet = 64 * pixelsInSampleSet1D * pixelsInSampleSet1D;
	m_randomSampler.Reset(samplesPerSet, c_NumSampleSets, Samplers::HemisphereDistribution::Cosine);
	std::vector<XMFLOAT4> SampleSets(samplesPerSet* c_NumSampleSets);
	...
	m_SampleSets.Create(L"SampleSets", (UINT)SampleSets.size(), sizeof(XMFLOAT4), SampleSets.data());

	m_CBRayGen.NumSamplesPerSet = m_randomSampler.NumSamples();
	m_CBRayGen.NumSampleSets = m_randomSampler.NumSampleSets();
	m_CBRayGen.NumPixelsPerDimPerSet = pixelsInSampleSet1D;
}

void CRayTracingAO::__UpdateGUI()
{
	ImGui::SetNextWindowSize(ImVec2(450, 150));
	ImGui::Begin("RayTracingAOGroundTruth_001(MultiSamplingAntiAliasing)");
	ImGui::SliderFloat("AoRadius", &m_CBRayGen.AORadius, 0.0f, 50.0f);
	ImGui::SliderFloat("MinT", &m_CBRayGen.MinT, 0.0f, 2.0f,"%.3f", 2.0f);
	ImGui::Text("AONumRays:%d", m_CBRayGen.AONumRays);
	ImGui::Checkbox("OpenMultiJitter", &m_CBRayGen.OpenMultiJitter);
	ImGui::End();
}